...
import com.example.lib_common.enent.ActivityClose;
...
public class MainLoginActivity extends BaseActivity {
    ...
    private Boolean entryTransiton = false;
...
//        transitionSet.addTransition(new ChangeImageTransform()); //处理ImageView的ScaleType属性
        ...
        transitionSet.addListener(new TransitionListenAdapter(){
            // 在动画结束后关闭splashActivity，使得界面平滑过度
            @Override
            public void onTransitionEnd(Transition transition) {
                super.onTransitionEnd(transition);
                entryTransiton = true;
                EventBus.getDefault().post(new ActivityClose(MainSplashActivity.class.getSimpleName())); // finish splashActivity
            }
        });
...
//        ObjectAnimator animator = ObjectAnimator.ofFloat(tv_login_sign,"textSize",20,30);
    ...
    public void onBackPressed() {
        // splashActivity finish后才能执行finish操作
        if(entryTransiton) {
            finish();
        }
    }
